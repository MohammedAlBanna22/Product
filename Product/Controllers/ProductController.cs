...
namespace Product.Controllers
{
    public class ProductController : Controller
    {
        ...
        public IActionResult Blogs()
        {
            ...
            IQueryable<Blogs> blogview = _context.Blogs.Where(x => string.IsNullOrEmpty(search) ? true :
            x.title.Contains(search)).AsQueryable();


            var blogs = blogview.Skip(skipe).Take(pageSize);
            var recordTotal = _rep.GetBlogs().Count();
            var jsonData = new { recordsFiltered = recordTotal, recordTotal, data = blogs };
            return Ok(jsonData);
        }
    }
}
